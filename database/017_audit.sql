...
(
    ...
    event_uuid UUID NOT NULL DEFAULT core.generate_uuid(),
    ...
    source_system VARCHAR(120) DEFAULT 'Kutlwano Platform',
    source_module VARCHAR(120),
    ip_address INET,
    user_agent TEXT,
    session_id UUID,
    correlation_id UUID,
    occurred_at TIMESTAMPTZ DEFAULT core.utc_now(),
    recorded_at TIMESTAMPTZ DEFAULT core.utc_now()
);
...
(
    ...
    transaction_timestamp TIMESTAMPTZ DEFAULT core.utc_now()
);

COMMENT ON TABLE audit.change_history
IS 'Historical row changes';

CREATE TABLE IF NOT EXISTS audit.record_versions
(
...
CREATE INDEX IF NOT EXISTS idx_record_versions
...
CREATE TABLE IF NOT EXISTS audit.security_audit
(
...
CREATE TABLE IF NOT EXISTS audit.compliance_audit
(
...
CREATE TABLE IF NOT EXISTS audit.configuration_audit
(
    ...
    change_reason TEXT,
    approved_by UUID,
    changed_at TIMESTAMPTZ DEFAULT core.utc_now()
);
...
CREATE TABLE IF NOT EXISTS audit.background_job_audit
(
    ...
    execution_id UUID DEFAULT core.generate_uuid(),
    execution_status VARCHAR(50) DEFAULT 'running',
    started_at TIMESTAMPTZ DEFAULT core.utc_now(),
...
CREATE TABLE IF NOT EXISTS audit.trigger_execution_audit
(
    ...
    error_message TEXT,
    executed_at TIMESTAMPTZ DEFAULT core.utc_now()
);

COMMENT ON TABLE audit.trigger_execution_audit
IS 'Database trigger execution history';

CREATE TABLE IF NOT EXISTS audit.file_integrity
(
    ...
    verification_date TIMESTAMPTZ DEFAULT core.utc_now(),
...
CREATE INDEX IF NOT EXISTS idx_file_integrity_document
ON audit.file_integrity(document_id);

CREATE TABLE IF NOT EXISTS audit.digital_evidence_chain
(
...
CREATE TABLE IF NOT EXISTS audit.tamper_detection
(
    ...
    detected_at TIMESTAMPTZ DEFAULT core.utc_now()
);

COMMENT ON TABLE audit.tamper_detection
IS 'Tamper detection register';

CREATE TABLE IF NOT EXISTS audit.forensic_cases
(
    ...
    opened_at TIMESTAMPTZ DEFAULT core.utc_now(),
...
CREATE TABLE IF NOT EXISTS audit.forensic_evidence
(
    ...
    collected_by UUID,
    collected_at TIMESTAMPTZ DEFAULT core.utc_now(),
    notes TEXT
);
...
CREATE TABLE IF NOT EXISTS audit.system_health
(
...
CREATE TABLE IF NOT EXISTS audit.timeline
(
...
CREATE TABLE IF NOT EXISTS audit.audit_analytics
(
...
CREATE INDEX IF NOT EXISTS idx_audit_analytics_date
ON audit.audit_analytics(reporting_date);

CREATE TABLE IF NOT EXISTS audit.dashboard_summary
(
...
CREATE OR REPLACE VIEW audit.v_audit_directory
AS
SELECT
    ae.audit_event_id,
    ae.module_name,
...
CREATE OR REPLACE VIEW audit.v_compliance_dashboard
AS
SELECT
    COUNT(*) AS total_reviews,
    COUNT(*) FILTER (WHERE compliant = TRUE) AS compliant,
    COUNT(*) FILTER (WHERE compliant = FALSE) AS non_compliant,
    ROUND(
...
CREATE OR REPLACE VIEW audit.v_security_dashboard
AS
SELECT
    COUNT(*) AS total_security_events,
    COUNT(*) FILTER (WHERE suspicious = TRUE) AS suspicious_events,
    COUNT(*) FILTER (WHERE login_result='failed') AS failed_logins,
    COUNT(*) FILTER (WHERE mfa_used=TRUE) AS mfa_logins
FROM audit.security_audit;

COMMENT ON VIEW audit.v_security_dashboard
IS 'Security dashboard';

CREATE OR REPLACE VIEW audit.v_forensic_dashboard
AS
SELECT
    COUNT(*) AS total_cases,
    COUNT(*) FILTER (WHERE investigation_status='open') AS open_cases,
...
CREATE OR REPLACE VIEW audit.v_system_health
AS
SELECT
...
CREATE OR REPLACE VIEW audit.v_global_search
AS
SELECT
    audit_event_id,
    module_name,
