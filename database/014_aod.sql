...
CREATE SCHEMA IF NOT EXISTS aod;

CREATE TABLE IF NOT EXISTS aod.aod_register
(
...
CREATE INDEX IF NOT EXISTS idx_aod_status
ON aod.aod_register(agreement_status);

CREATE INDEX IF NOT EXISTS idx_aod_attorney
ON aod.aod_register(attorney_id);

CREATE TABLE IF NOT EXISTS aod.agreements
(
...
CREATE TABLE IF NOT EXISTS aod.installments
(
    ...
    installment_status VARCHAR(50) DEFAULT 'pending'
);
...
CREATE TABLE IF NOT EXISTS aod.payments
(
...
CREATE TABLE IF NOT EXISTS aod.payment_allocations
(
...
CREATE TABLE IF NOT EXISTS aod.interest_calculations
(
    ...
    calculated_at TIMESTAMPTZ DEFAULT core.utc_now()
);

COMMENT ON TABLE aod.interest_calculations
IS 'Interest calculations';

CREATE TABLE IF NOT EXISTS aod.penalty_calculations
(
...
CREATE TABLE IF NOT EXISTS aod.payment_reminders
(
...
CREATE TABLE IF NOT EXISTS aod.debt_balances
(
...
CREATE TABLE IF NOT EXISTS aod.collection_cases
(
    ...
    collection_stage VARCHAR(50) DEFAULT 'friendly_reminder',
    priority master.case_priority DEFAULT 'normal',
...
CREATE INDEX IF NOT EXISTS idx_collection_case_status
ON aod.collection_cases(collection_status);

CREATE TABLE IF NOT EXISTS aod.collection_actions
(
    ...
    performed_by UUID,
    performed_at TIMESTAMPTZ DEFAULT core.utc_now()
);

COMMENT ON TABLE aod.collection_actions
IS 'Collection activities';

CREATE TABLE IF NOT EXISTS aod.promise_to_pay
(
    ...
    honoured BOOLEAN DEFAULT FALSE,
    honoured_date DATE,
...
CREATE TABLE IF NOT EXISTS aod.defaults
(
...
CREATE TABLE IF NOT EXISTS aod.legal_escalations
(
    ...
    escalated_by UUID,
    escalated_at TIMESTAMPTZ DEFAULT core.utc_now()
);

COMMENT ON TABLE aod.legal_escalations
IS 'Legal escalation register';

CREATE TABLE IF NOT EXISTS aod.settlement_offers
(
...
CREATE TABLE IF NOT EXISTS aod.documents
(
...
CREATE TABLE IF NOT EXISTS aod.digital_signatures
(
...
CREATE TABLE IF NOT EXISTS aod.internal_notes
(
...
CREATE TABLE IF NOT EXISTS aod.timeline
(
...
CREATE TABLE IF NOT EXISTS aod.audit_trail
(
...
CREATE TABLE IF NOT EXISTS aod.collection_kpi
(
...
CREATE INDEX IF NOT EXISTS idx_collection_kpi_month
ON aod.collection_kpi(reporting_month);

CREATE TABLE IF NOT EXISTS aod.payment_analytics
(
...
CREATE TABLE IF NOT EXISTS aod.dashboard_summary
(
...
CREATE TABLE IF NOT EXISTS aod.escalation_queue
(
...
CREATE TABLE IF NOT EXISTS aod.reminder_queue
(
...
CREATE OR REPLACE VIEW aod.v_aod_directory
AS
SELECT
    a.aod_id,
    a.aod_number,
...
CREATE OR REPLACE VIEW aod.v_dashboard
AS
SELECT
    COUNT(*) AS total_accounts,
    COUNT(*) FILTER (WHERE agreement_status='active') AS active_accounts,
    COUNT(*) FILTER (WHERE agreement_status='completed') AS completed_accounts,
    COUNT(*) FILTER (WHERE agreement_status='defaulted') AS defaulted_accounts,
